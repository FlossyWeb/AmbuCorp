...
function active()
{
	var posting = $.post("https://ssl14.ovh.net/~taxibleu/ambuclient/active_app.php", { tel: tel, mngid: mngid, dep: 'ambu'}, function(data) {
...
function secureCall(position)
{
	...
	$.post("https://ssl14.ovh.net/~taxibleu/ambuclient/secure_xml.php", { lat: lat, lng: lng, dep: 'ambu', pass: 'true'}, function(xml){
		...
		$(xml).find('marker').each(function(){
			...
			$.post("https://ssl14.ovh.net/~taxibleu/ambuclient/secure.php", { taxi: name, tel: address, rdvpoint: rdvpoint, helptaxi: taxi, helpname: helpname, helptel: tel, idcourse: idcourseUrg, num_req: num_reqUrg, dep: 'ambu', pass: 'true'}, function(data){
...
{
	var idcourseUrg = $.sessionStorage.getItem('idcourseUrg');
	$.post("https://ssl14.ovh.net/~taxibleu/ambuclient/secure.php", { taxi: '', tel: '', rdvpoint: '', helptaxi: taxi, helpname: '', helptel: tel, idcourse: idcourseUrg, dep: 'ambu', pass: 'true', stopcall: 'true'}, function(data){
...
$(document).bind( 'pagecreate', function() {
	...
	$("#login").submit(function(event) {
		...
		// Subs some data 
		$.post("https://ssl14.ovh.net/~taxibleu/ambuclient/login_app.php", $("#login").serialize(), function(data) {
			// GET SHIT BACK !!
			$.localStorage.setItem('civil', data.civil);
			$.localStorage.setItem('nom', data.nom);
			$.localStorage.setItem('prenom', data.prenom);
	...
	$("#forget").submit(function(event) {
		...
		event.preventDefault();
		// Subs some data
		$.post("https://ssl14.ovh.net/~taxibleu/ambuclient/forget_app.php", $("#forget").serialize(), function(data) {
	...
	$("#register").submit(function(event) {
		...
		{	
			...
			$.mobile.loading( "show" );
			// Subs some data
			$.post("https://ssl14.ovh.net/~taxibleu/ambuclient/register_app.php", $("#register").serialize(), function(data) {
				// GET SHIT BACK !!
				$.localStorage.setItem('civil', data.civil);
				$.localStorage.setItem('nom', data.nom);
				$.localStorage.setItem('prenom', data.prenom);
